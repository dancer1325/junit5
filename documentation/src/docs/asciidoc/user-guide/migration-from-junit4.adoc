...
* `@BeforeClass` and `@AfterClass` no longer exist; use `@BeforeAll` and `@AfterAll`
  instead.
* `@Ignore` NO longer exists
    ** use `@Disabled` OR one of the other built-in <<writing-tests-conditional-execution, execution conditions>>
    **  check <<migrating-from-junit4-ignore-annotation-support>>
...
* `@Test(expected = ...)` & `ExpectedException` rule NO longer exist
    ** use `Assertions.assertThrows(...)` instead
    ** check <<migrating-from-junit4-rule-support>>
