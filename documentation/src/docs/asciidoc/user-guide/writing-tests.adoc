...
* Supported by JUnit Jupiter for
    ** configuring tests
    ** extending the framework

* ALL core annotations are | `{api-package}` package's  `junit-jupiter-api` module
...
| `@TestClassOrder`        | allows configuring the <<writing-tests-test-execution-order-classes, test class execution order>> -- for -- `@Nested` test classes / are inherited
...
| `@Nested`                | == class is NON-static <<writing-tests-nested,nested test class>>

From Java 8 to Java 15, if you want to use `@BeforeAll` and `@AfterAll` in a `@Nested` test class -> you need to use "per-class" <<writing-tests-test-instance-lifecycle, test instance lifecycle>>

From Java v16+, `@BeforeAll` and `@AfterAll` methods can be declared as `static` in a `@Nested` / are NOT inherited
...
****
Container::
* := node | test tree / contains other
    ** containers OR
    ** tests as its children -- _Example:_ _test class_

Test::
* := node | test tree / once it's executed -> verify expected behavior
    ** _Example:_ `@Test` method
****

.Jupiter Concepts
****
Lifecycle Method::
* := method / directly annotated or meta-annotated with
    ** `@BeforeAll`
    ** `@AfterAll`,
    ** `@BeforeEach`
    ** `@AfterEach`
...
Test Class::
* := top-level class OR `static` member class OR <<writing-tests-nested,
`@Nested` class>> / contains >=1 _test method_
    ** == _container_
    ** requirements
        *** NOT be `abstract`
        *** have a 1 constructor

Test Method::
* := instance method / directly annotated or meta-annotated with
    ** `@Test`,
        *** ⚠️ NOT create a container | test tree ⚠️
    ** `@RepeatedTest`,
        *** create a container | test tree
    ** `@ParameterizedTest`,
        *** create a container | test tree
    ** `@TestFactory`,
        *** create a container | test tree
...
* allowed |
    ...
    ** methods
* uses
    ** filter <<running-tests, test discovery and execution>>
* check
    ** <<running-tests-tags>>
    ** <<writing-tests-meta-annotations>>
...
* Test classes & methods -- are 👁️ordered 👁️ via -- deterministic algorithm, BUT NOT obvious
    ...
    ** Check <<writing-tests-definitions>>

[[writing-tests-test-execution-order-methods]]
==== Method Order

* true _unit tests_
    ** 👁️ should NOT rely on the order / they are executed 👁️
* use cases
    ** writing _integration tests_ or _functional tests_ / sequence of the tests is important
    ** previous + `@TestInstance(Lifecycle.PER_CLASS)`
* `{TestMethodOrder}` & `{MethodOrderer}` | test class or test interface
    ** allowed `{MethodOrderer}`
        *** create your own custom one
        *** built-in implementations
            **** `{MethodOrderer_DisplayName}`
                ***** sorts test methods _alphanumerically_ -- based on -- their
display names
                ***** Check <<writing-tests-display-name-generator-precedence-rules, display name generation precedence rules>>
            **** `{MethodOrderer_MethodName}`
                ***** sorts test methods _alphanumerically_ -- based on -- their names & formal parameter lists
            **** `{MethodOrderer_OrderAnnotation}`
                ***** sorts test methods _numerically_ -- based on -- values specified via `{Order}` annotation
            **** `{MethodOrderer_Random}`
                ***** sort orders test methods _pseudo-randomly_ / -- supports --
  configuration of a custom _seed_
            **** `{MethodOrderer_Alphanumeric}`
                *****  **deprecated -- in favor of -- `{MethodOrderer_MethodName}`/ will be removed | 6.0**
                ***** sorts test methods _alphanumerically_ -- based on their --
  names & formal parameter lists
    ** Check <<extensions-execution-order-wrapping-behavior>>
    ** _Example:_
...
* `junit.jupiter.testmethod.order.default`
    ** <<running-tests-config-params>> / specify the fully qualified class name of the
`{MethodOrderer}` / -- has to implement the --
`MethodOrderer` interface
    ** apply for ALL test, except to, test class or interface / `@TestMethodOrder` annotation
* _Example:_ src/test/resources/junit-platform.properties
...
* allows
    ** expressing the relationship among several groups of tests
* how does it work?
    ** use Java's nested classes / full lifecycle support
        *** EXCEPT to, if Java < v16, for `@BeforeAll` and `@AfterAll`
            **** Reason: 🧠 Java does NOT allow `static` members | inner classes  🧠
            **** workaround: `@Nested` + `@TestInstance(Lifecycle.PER_CLASS)` | test class -- Check <<writing-tests-test-instance-lifecycle>> --
        *** From Java v16+ -> `@BeforeAll` and `@AfterAll` -- must be declared as -- `static`
* _Example:_
    ...
    ** test execution | IDE -> looks like a tree in the GUI

image::writing-tests_nested_test_ide.png[caption='',title='Executing a nested test in an IDE']

* setup code from outer tests -- is run before -- inner tests
* requirements to be a `@Nested` test class
...
* | ALL prior JUnit versions
    ** ⚠️test constructors or methods & standard `Runner` implementations -- were -- NOT allowed to have parameters ⚠️
* | JUnit Jupiter
    ** 👀test constructors & methods are NOW permitted to have parameters 👀
        *** ONE of the major changes
        *** -> greater flexibility & enables _Dependency Injection_ | constructors and methods

* `{ParameterResolver}`
    ** see <<extensions#extensions-parameter-resolution>>
    ** built-in resolvers / registered automatically
        *** `{TestInfoParameterResolver}`
            **** if a constructor or method parameter is of type
  `{TestInfo}` -> `TestInfoParameterResolver` -- will supply, as value for the parameter, an -- instance of `TestInfo` / -- corresponding to the -- current container or test
                ***** `TestInfo` uses
                    ****** retrieve information about the current container
                    ****** test -- _Example:_ display name (== technical name or a custom name configured via `@DisplayName`), test class, test method, associated tags --
                ***** `{TestInfo}` == drop-in replacement for the `TestName` rule | JUnit 4
***** _Example:_ inject `TestInfo` | test constructor, `@BeforeEach` method, and `@Test` method
+
[source,java,indent=0]
----
...
----
        ...
        *** `{RepetitionExtension}`
            **** if a method parameter is of type `{RepetitionInfo}` | `@RepeatedTest`, `@BeforeEach`, or
  `@AfterEach` -> `RepetitionExtension` -- will supply an -- instance of `RepetitionInfo`
                ***** `RepetitionInfo` uses
                    ****** retrieve information about the current repetition, total number of repetitions, number of
  repetitions / have failed, failure threshold for the corresponding
  `@RepeatedTest`
                ***** `RepetitionExtension` is NOT registered -- outside the context of a -- `@RepeatedTest`
...
* `@RepeatedTest`
    ** allowed | JUnit Jupiter
    ** allows
        *** 👀specifying the # of repetitions of a test 👀
            **** EACH invocation == execution of a regular `@Test` method / FULL support for the SAME lifecycle callbacks & extensions
    ** _Example:_
+
[source,java]
----
@RepeatedTest(10)   // 10 times will be repeated
void repeatedTest() {
	// ...
}
----

    ** | JUnit Jupiter v5.10+, `@RepeatedTest(failureThreshold)` -- can be configured with a -- failure threshold
        *** 👀== number of failures AFTER remaining repetitions -> will be automatically skipped 👀
        *** < total number of repetitions
            **** Reason: 🧠 skip the invocations of remaining repetitions 🧠
        *** use cases
            **** test / you suspect to be _flaky_
                ***** Reason: 🧠1! failure is sufficient 🧠
        *** by default, `Integer.MAX_VALUE`
            **** == NO failure threshold -- will be -- applied -> ALL specified number of repetitions will be invoked
        *** 👀if the repetitions are executed in parallel -> NO guarantees -- regarding the -- failure threshold 👀
            **** -> recommended to annotate with `@Execution(SAME_THREAD)`
            **** see <<writing-tests-parallel-execution>>
    ** configure the custom display name / EACH repetition -- via the -- `name` attribute
        *** can be a pattern == static text + dynamic placeholders /
            **** supported placeholders
                ***** `{displayName}` == display name of the `@RepeatedTest` method
                ***** `{currentRepetition}`== current repetition count
                ***** `{totalRepetitions}` == total number of repetitions
        *** default display name / given repetition == `"repetition {currentRepetition} of {totalRepetitions}"`
            **** _Example:_ (based on previous @RepeatedTest) `repetition 1 of 10`, `repetition 2 of 10`, etc.
        *** predefined `RepeatedTest.LONG_DISPLAY_NAME` pattern == `"{displayName} :: repetition {currentRepetition} of {totalRepetitions}"`
            **** _Example:_ (based on previous @RepeatedTest) `repeatedTest() :: repetition 1 of 10`, `repeatedTest() :: repetition 2 of 10`, etc.
    ** information / can be retrieved
        *** current repetition,
        *** total number of repetitions,
        *** number of repetitions / failed,
        *** failure threshold,
    ** ways to retrieve previous information
        *** instance of `{RepetitionInfo}` injected |
            **** `@RepeatedTest`,
            **** `@BeforeEach`,
            **** `@AfterEach`
...
* allows
    ** run 1 test multiple times / different arguments
* requirements
    ** declare >= 1 _source_ /
        *** arguments / invocation
        *** _consume_ / test method
        *** _Example:_ `@ValueSource` / ALL possible properties or arguments are arrays
...
* 1 separately execution test repot / invocation
    ** _Example:_
...
* add a dependency | `junit-jupiter-params` artifact
    ** Check <<dependency-metadata>>

[[writing-tests-parameterized-tests-consuming-arguments]]
==== Consuming Arguments

* Parameterized test methods' allowed _consume_ arguments
    ** directly -- from the -- configured source
        *** Check <<writing-tests-parameterized-tests-sources>>
        *** 1to1 : argument source index -- method parameter index
        *** _Examples:_ <<writing-tests-parameterized-tests-sources-CsvSource>>)
    ** _aggregate_ arguments -- from the -- source | 1! object /
        *** can be
            **** -- passed to the -- method OR
            **** -- provided by -- `ParameterResolver`
        *** Check <<writing-tests-parameterized-tests-argument-aggregation>>)
* rules to declare formal parameters / order must be
    ** >=0 _indexed arguments_
    ** >=0 _aggregators_
    ** >=0 arguments supplied by a `ParameterResolver`
* _indexed argument_
    ** := argument / given index | `Arguments` -- provided by an -- `ArgumentsProvider`
        *** -- passed as an -- argument to the parameterized method
        *** index == index | method's formal parameter list
* _aggregator_
    ** := ANY parameter of type `ArgumentsAccessor` or ANY parameter / annotated with `@AggregateWith`
* AutoCloseable arguments
    ** == arguments / implement `java.lang.AutoCloseable` (or `java.io.Closeable` & extends `java.lang.AutoCloseable`)
        *** automatically closed | after calling `@AfterEach` methods and
`AfterEachCallback` | current parameterized test
    ** `@ParameterizedTest(autoCloseArguments=false)`
        *** prevent the closure automatically
        *** uses
            **** argument / implements `AutoCloseable` & reused  multiple invocations of the same parameterized test method
                ***** Reason: 🧠 -> argument is NOT closed between invocations 🧠

[[writing-tests-parameterized-tests-sources]]
==== Sources of Arguments

* built-in _source_ annotations
    ** Check next subsections & `{params-provider-package}`
...
* allows
    ** referring to >=1 _factory_ methods | test class or external classes
        *** Factory methods | test class
            **** if test class NOT annotated with `@TestInstance(Lifecycle.PER_CLASS)` -> must be `static`
        *** factory methods | external classes
            **** must ALWAYS be `static`
* return a _stream_ of _arguments_
    ** _stream_ | JUnit == ANYTHING / JUnit -- can convert into a -- `Stream`
        *** `Stream<Arguments>`
        *** `DoubleStream<Arguments>`
        *** `LongStream<Arguments>`
        *** `IntStream<Arguments>`
    ...
    ** _arguments_ | JUnit can be
        *** `Arguments`
        *** `Object`
    ** uses
        *** arguments for individual invocations of the annotated `@ParameterizedTest` method
* 👁️if you do NOT explicitly provide a factory method name via `@MethodSource` -> JUnit Jupiter
will search for a _factory_ method / name == `@ParameterizedTest` method 👁️
...
* := API /
    ** allows
        *** ⭐ accessing to the provided arguments -- through -- 1! argument passed ⭐
    ** support type conversion -- check <<writing-tests-parameterized-tests-argument-conversion-implicit>> --
    ** `getInvocationIndex()`
        *** retrieve the current test invocation index
    ** 1 instance of `ArgumentsAccessor` injected automatically / ANY parameter of type `ArgumentsAccessor`
* use cases
    ** 👁️alternative to add MULTIPLE parameters 👁️
        *** Reason: 🧠 1 method parameter / (by default) `@ParameterizedTest` method ' s arguments
            **** -> if you want to pass many `@ParameterizedTest` 's arguments -> large method signatures
...
* steps
    ** implement the `{ArgumentsAggregator}`
        *** 👁️ UNIQUE ways to declare them 👁️
            **** top-level class OR
            **** `static` nested class
    ** register it -- via the -- `@AggregateWith` | `@ParameterizedTest` method's parameter
...
* custom _composed annotation_ /
    ** steps
        *** implement the `{ArgumentsAggregator}`  -- as another custom aggregator --
        *** 👁️ create annotation / meta-annotated with `@AggregateWith(MyTypeAggregator.class)` 👁️
    ** use cases
        *** avoid REPEATEDLY registering custom aggregators ( == `@AggregateWith(MyTypeAggregator.class)` ) | multiple parameterized test methods
...
* JUnit Jupiter
    ** 👁️ by default, tests are run sequentially | 1! thread 👁️
* pros
    ** speed up execution
* requirements
    ** v5.3+
* how to set up?
    ** `junit.jupiter.execution.parallel.enabled = true`
        *** Check <<running-tests-config-params>>
            **** _Example:_ | `junit-platform.properties`
    ** set nodes ( | test tree) ' execution mode to concurrent
        *** ways
            **** `junit.jupiter.execution.parallel.mode.default = concurrent` or
            **** `@Execution`
                ***** | element -- affects to -> annotated element + its subelements
            **** `junit.jupiter.execution.parallel.mode.classes.default = concurrent`
                ***** == default execution mode | top-level classes
                ***** ⚠️ if it's NOT explicitly set -> inherit the value from `junit.jupiter.execution.parallel.mode.default`  ⚠️
        *** available execution modes
            **** `SAME_THREAD`
                ***** default by ALL test / EXCEPTION
                    ****** test classes / use `Lifecycle.PER_CLASS` mode
                    ****** test classes / use `MethodOrderer`, EXCEPT to `MethodOrderer_Random`
                ***** test's thread == parent's thread
                    ****** _Example:_ (ALL next methods | same test class) `@BeforeAll` method's thread == ANY method's thread == `@AfterAll` method's thread
                ***** + default `ClassOrderer` registered (Check <<writing-tests-test-execution-order-classes>>) -> top-level test classes scheduled and sorted accordingly, BUT NOT guaranteed to start from that order
                    ****** Reason: 🧠 threads they are executed on are NOT controlled by JUnit 🧠
            **** `CONCURRENT`
                ***** == tests methods run concurrently
                    ****** EXCEPTION  resource lock forces execution | SAME thread
                ***** ALL nodes | tree / configured concurrently
                    ****** executed fully in parallel / -- according to the provided -- <<writing-tests-parallel-execution-config, configuration>>
                    ****** observe the declarative declarative <<writing-tests-parallel-execution-synchronization, synchronization>> mechanism
                    ****** <<running-tests-capturing-output>> -- needs to be enabled -- separately
* _Examples:_
    ** _Example1:_
    ...
    ** _Example4:_ diagram / 2 top-level test classes `A` and `B` / 2 test methods / class / ALL 4 combinations of
...
* `{ParallelExecutionConfigurationStrategy}`
    ** allows
        *** configuring properties /
            **** desired parallelism
            **** maximum pool size
    ** implementations -- `junit.jupiter.execution.parallel.confi.strategy=implementationName`
        *** built-in
            **** `dynamic`
                ***** desired parallelism = # of available processors/cores * `junit.jupiter.execution.parallel.config.dynamic.factor`
                    ****** by default, `junit.jupiter.execution.parallel.config.dynamic.factor=1`
                ***** `junit.jupiter.execution.parallel.config.dynamic.max-pool-size-factor` limit the maximum # of threads
                ***** default strategy
            **** `fixed`
                ***** desired parallelism = `junit.jupiter.execution.parallel.config.fixed.parallelism`
                ***** `junit.jupiter.execution.parallel.config.fixed.max-pool-size` limit the maximum # of threads
        *** `custom` / can be implemented
            **** `junit.jupiter.execution.parallel.config.custom.class` specify the desired configuration

* ONLY Parallelism NOT -> maximum # of concurrent threads
    ** Reason: 🧠 JUnit Jupiter does NOT guarantee / # of concurrently
executing tests < configured parallelism 🧠
        *** _Example:_ if you use `ForkJoinPool` -- as -- synchronization mechanisms -> additional threads may be spawn / execution with  sufficient parallelism
    ** Solution: Java 9+ & parallel execution strategies (`dynamic`, `fixed` or `custom`) / specify the maximum pool size of the  strategies

[[writing-tests-parallel-execution-config-properties]]
===== Relevant properties for configuring parallel execution
...
| Execution parallelism strategy & maximum pool size
|
  * `dynamic`
  * `fixed`
  * `custom`
...
| * # of available processors/cores = desired parallelism for the ```dynamic``` configuration strategy
| a positive decimal number
| ```1.0```

| ```junit.jupiter.execution.parallel.config.dynamic.max-pool-size-factor```
| * # of available processors/cores * `junit.jupiter.execution.parallel.config.dynamic.factor` = desired
  parallelism for the ```dynamic``` configuration strategy
| a positive decimal number / -- must be -- >= `1.0`
...
| a positive integer / -- must be -- >= `junit.jupiter.execution.parallel.config.fixed.parallelism`
...
| == Fully qualified class name of the _ParallelExecutionConfigurationStrategy_ for the ```custom``` configuration strategy
| _Example:_ _org.example.CustomStrategy_
| no default value
|===
...
* `{ResourceLock}`
    ** := synchronization mechanism /
        *** == test class OR method -- uses a -- specific shared resource /
            **** -- requires -- synchronized access
            **** -- is identified by an -- unique name / can be
                ***** user-defined OR
                ***** predefined constants | `{Resources}`
                    ****** _Example:_ `SYSTEM_PROPERTIES`, `SYSTEM_OUT`, `SYSTEM_ERR`, `LOCALE`, or `TIME_ZONE`
            **** access mode can be specified
                ***** 2 tests / -- require `READ` access to a -- shared resource ->  can run |
parallel
                ***** 👁️2 tests / -- 1 of them require `READ_WRITE` access to a -- shared resource ->  can NOT run | parallel 👁️
...
----
    ...
    ** if tests run in parallel & NOT {ResourceLock} ->  _flaky_ test
        *** _flaky_ test == sometimes pass & then fail / -- due to -- writing and reading | same JVM's System Property
        *** == ⭐ if tests run in parallel & shared resources -> you need to use `{ResourceLock}` ⭐
            **** Reason: 🧠 JUnit Jupiter engine -- uses it to ensure that -- NO conflicting tests running | parallel 🧠
    ** "lock" is acquired [before `@BeforeEach` methods, after `@AfterEach` methods]
* `{Isolated}`
    ** 👁️:= annotation | test classes / those class' methods are executed sequentially 👁️
    ** use case
        *** most of your test classes can be run | parallel / NO synchronization + SOME test classes / need to run | isolation
