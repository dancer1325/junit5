...
* ways to register
    ** _declaratively_
        *** -- via -- <<extensions-registration-declarative,`@ExtendWith`>>
    ** _programmatically_
        *** -- via -- <<extensions-registration-programmatic,`@RegisterExtension`>>
    ** _automatically_
        *** -- via -- Java's <<extensions-registration-automatic,`ServiceLoader`>> mechanism

[[extensions-registration-declarative]]
==== Declarative Extension Registration

* allows
    ** registering >= 1 extensions
* `@ExtendWith(classReference1, classReference2, ...)`
    ** == annotation -> UNIQUE way to register declaratively
    ** |
        *** test interface,
        *** test class,
        *** test method,
        *** custom _<<writing-tests-meta-annotations,composed annotation>>_
        *** test class constructors' fields or parameters
            **** from JUnit Jupiter v5.8+
            **** | superclasses' fields -- will be -- inherited
                ***** Check <<extensions-registration-inheritance, Extension Inheritance>>
        *** lifecycle methods -- `@BeforeAll`, `@AfterAll`, `@BeforeEach`, and `@AfterEach` --
            **** from JUnit Jupiter v5.8+
    ** 👁️ there is a registration order -- based on -- declaration | source code 👁️
        *** | fields
            **** by default, it's ordered -- via an -- algorithm /
                ***** is deterministic BUT intentionally nonobvious
                ***** ALL runs of a test suite execute extensions / SAME order
            **** -- can be ordered via -- `@Order`
                ***** Check <<extensions-registration-programmatic-order, Extension Registration Order>>
    ** -- NOT supported to register --  `TestInstancePostProcessor` | NON-static field
        *** Check link:{junit5-repo}/issues/3437[issue 3437]
        *** implements `BeforeEachCallback` -- as an -- alternative
    ** 's fields -- may be -- `static` or NON-static
        *** == `@RegisterExtension` 's fields ->  Check
...
* _Example1:_ let's `WebServerExtension` / starts a local web server & injects the server's URL | parameters / -- annotated with -- `@WebServerUrl`
    ** _Example1.1:_ register a `WebServerExtension` | test method
...
----
    ...
    ** _Example1.2:_ register the `WebServerExtension` | specific class -> ALL class' & subclass' tests
...
* _Example2:_ register multiple extensions, -- taking in account the -- order declaration
    ** _Example2.1:_ together

[source,java,indent=0]
----
@ExtendWith({ DatabaseExtension.class, WebServerExtension.class })
// extension orderly DatabaseExtension and WebServerExtension
class MyFirstTests {
	// ...
}
----

    ** _Example2.2:_ separately
...
* _Example3:_ register extension | field or parameter level
...
* Composed annotation / use `@ExtendWith` as _meta-annotation_
    ** allows
        *** 👁️ multiple extensions -- are combined in a -- reusable way 👁️
    ** Check _<<writing-tests-meta-annotations,composed annotation>>_
    ** _Example:_ `@ExtendWith({ DatabaseExtension.class, WebServerExtension.class })` -- is replaced by -- `@DatabaseAndWebServerExtension`
...
* `{RegisterExtension}`
    ** allows
        *** registering extensions _programmatically_ | test classes' fields
    ** registration order | fields
        *** by default == `@ExtendWith`
            **** == if you do NOT specify order | field -> default order
            **** default order value = `Integer.MAX_VALUE / 2`
        *** -- allows -- specifying order | fields
            **** == `@Order` | `@ExtendWith`
            **** if order value
                ***** < default order value ->
                    ***** registered BEFORE NON-annotated extensions
                    ***** _after_ callback extensions -- are registered -- last
                    ***** _before_ callback extensions -- are registered -- first
                ***** > default order value ->
                    ***** registered AFTER NON-annotated extensions
                    ***** _after_ callback extensions -- are registered -- first
                    ***** _before_ callback extensions -- are registered -- last
    ** extensions | superclasses' fields -- will be -- inherited
        *** Check <<extensions-registration-inheritance, Extension Inheritance>>
    ** 's fields
        *** must NOT be `null`
...
* Auto-detection
    ** == advanced feature / 👀NOT enabled by default 👀
    ** if you want to enable it -> set `junit.jupiter.extensions.autodetection.enabled=true`
        *** ways to supply
            **** as a JVM system property, -- `-Djunit.jupiter.extensions.autodetection.enabled=true` --
            **** as a _configuration parameter_ | `LauncherDiscoveryRequest`
            **** JUnit Platform configuration file (see <<running-tests-config-params>>)
        *** extensions / discovered via the `{ServiceLoader}` mechanism -> will be added | extension registry / AFTER JUnit Jupiter's global extensions (_Example:_ support for `TestInfo`, `TestReporter`, ...)
...
* `{ParameterResolver}`
    ** 👀== `Extension` API -- for dynamically resolving -- parameters | runtime 👀
    ** can be
        *** built-in (see `{TestInfoParameterResolver}`) or
        *** <<extensions-registration,registered by the user>>

* if a _test class_ constructor, _test method_, or _lifecycle method_ declares a parameter -> parameter MUST be _resolved_ -- by a -- `ParameterResolver` | runtime
    ** see <<writing-tests-definitions>>
    ** ways to resolve a parameter
        *** by _name_,
        *** by _type_,
            **** if you wish to implement a custom `{ParameterResolver}` / resolves parameters -- based SOLELY on the -- type -> extend the `{TypeBasedParameterResolver}`
            **** see `{CustomTypeParameterResolver}`
        *** by _annotation_
            **** see `{CustomAnnotationParameterResolver}`
        *** combination of previous ones
            **** see `{MapOfListsTypeBasedParameterResolver}`

* if you use JDK v9- & look up annotations on parameters -- via the -- core `java.lang.reflect.Parameter` API -> will ALWAYS fail | _inner class_ constructors (_Example:_ `@Nested`
test class constructor)
    ** Reason: 🧠 bug | byte code -- generated by -- `javac` | JDK v9-
    ** -> `{ParameterContext}` API / supplied to `ParameterResolver` implementations -> includes methods for looking up annotations | parameters (recommended ones)
        *** `boolean isAnnotated(Class<? extends Annotation> annotationType)`
        *** `Optional<A> findAnnotation(Class<A> annotationType)`
        *** `List<A> findRepeatableAnnotations(Class<A> annotationType)`

* OTHER extensions
    ** can also leverage registered `ParameterResolvers` | method & constructor invocations
        *** -- via the -- `ExtensionContext.getExecutableInvoker()` & `{ExecutableInvoker}`
