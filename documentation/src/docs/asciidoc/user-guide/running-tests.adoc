...
* Maven
Surefire and Maven Failsafe
    ** from https://issues.apache.org/jira/browse/SUREFIRE-1330[v2.22.0] -> -- https://maven.apache.org/surefire/maven-surefire-plugin/examples/junit-platform.html[native support]
for executing -- tests | JUnit Platform
    ** alternatively, add a proper version of
...
`{junit5-jupiter-starter-maven}`
        *** how to use the Maven Surefire plugin
    ** v3.0.0-M4+ fix interoperability issues
        *** Check https://issues.apache.org/jira/browse/SUREFIRE-1585[introduced support]
        *** align version of the JUnit Platform Launcher -- with -- JUnit Platform version | test runtime classpath
...
* JUnit Platform <<running-tests-config-params, configuration parameters>>
* allows
    ** influence
        *** test discovery
        *** execution
* `configurationParameters` property / pass key-value pairs
    ** key-value pairs -- are passed, following the -- syntax
        *** Java `.properties` or
...
* `JUnitPlatform`
    ** := runner /
        *** -- created for -- running | JUnit Platform | JUnit 4 environment
            **** test suites
            **** tests
    ** 👁️ + `@UseTechnicalNames` ALREADY deprecated -- by -- `@Suite`, -- from -- JUnit Platform v1.8 👁️
        *** Reason1: 🧠 currently, ALL tests run | (directly) JUnit Platform  🧠
        *** Reason2: 🧠 after the introduction of `@Suite` -> `JUnitPlatform` runner is obsolete 🧠
    ** 👁️ will be removed -- from -- JUnit Platform v2.0+ 👁️
* `@RunWith(JUnitPlatform.class)` | class -> allows running tests /
    ** support JUnit 4
    ** NOT yet support JUnit Platform directly
* TODO:
...
* configuration options -- for -- discovering and filtering tests
    ** check Javadoc of the `{suite-api-package}` package
    ** _Example:_ `@SelectPackages`
* if test classes and suites / -- annotated with -- `@RunWith(JUnitPlatform.class)` ->
    ** ⚠️ can NOT be executed directly | JUnit Platform (as a "JUnit 5" test) ⚠️
    ** can ONLY be executed | JUnit 4

[[running-tests-discovery-selectors]]
=== Discovery Selectors

* provided by JUnit Platform
* allows
    ** specifying tests / should be
        *** discovered
        *** executed
* ways to created them
    ** programmatically -- via -- `{DiscoverySelectors}` class' factory methods
    ** declaratively -- via --
        *** annotations | <<junit-platform-suite-engine>>
        *** options | <<running-tests-console-launcher>>
...
* := key-value pairs /
    ** both are text-based
    ** 👁️ ways to supply to test engines / running | JUnit Platform, ⚠️taking precedence from first to last of next options (_Example:_ `Launcher` takes precedence over JVM)  ⚠️ 👁️
        *** `LauncherDiscoveryRequestBuilder.configurationParameter()` & `LauncherDiscoveryRequestBuilder.configurationParameters()`
            **** build a request / -- supplied to the -- <<launcher-api, `Launcher` API>>
            **** allowed configuration parameters
                ***** CL option `--config` -- Check <<running-tests-console-launcher,Console Launcher>> --
                ***** `systemProperty` or `systemProperties` DSL -- Check <<running-tests-build-gradle-config-params,Gradle>> --
                ***** `configurationParameters` property -- Check <<running-tests-build-maven-config-params,Maven Surefire provider>> --
        *** JVM system properties
        *** `junit-platform.properties`
            **** JUnit Platform configuration file
            **** | root of the class path
            **** syntax rule == Java '.properties' syntax rule
* can be specific to a particular
    ** test engine,
        *** _Example:_ JUnit Jupiter `TestEngine` supports _configuration parameters_ | use cases
...
* := JUnit Platform concept -- for -- marking and filtering tests
    ** -- is defined by the -- testing framework
        *** _Example1:_ `@Tag` | JUnit Jupiter based tests
            **** check <<writing-tests-tagging-and-filtering>>
        *** _Example2:_ `@Category` | JUnit 4 based tests / Vintage engine
            **** check <<migrating-from-junit4-categories-support>>
...
NOTE: "trimmed" = removing leading and trailing whitespace
...
* == boolean expressions /
    ** allowed operators
        ...
        *** `(`  `)`
    ** special expressions
        *** `any()` == ALL tests / have tags
        *** `none()` ==  ALL tests / NO have tags
...
* _Example:_ tagging by
    ** test type ( -- _Example:_ _micro_, _integration_, _end-to-end_ --)
    ** feature (-- _Example:_ *product*, *catalog*, *shipping* --)
